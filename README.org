...
**IP of the storage nodes are hardcoded in `shared-headers/Defs.hpp`. Change the IP addresses to match your setup.**
...
#+begin_src shell
sudo apt install libaio-dev libnuma-dev
mkdir build
cd build
cmake ..
make -j$(nproc)
...
#+begin_src shell
./build/frontend/optmistic_benchmark -ownIp=10.10.1.1 -storage_node -dramGB=10 -all_worker 16 -worker 16
#+end_src

Starting the client nodes:
#+begin_src shell
./build/frontend/optmistic_benchmark -ownIp=10.10.1.2 -run_for_seconds=10 -pinThreads -csvFile="optimistic_benchmark_scaleout.csv" -readratios 100 -broken -block_size 64 -worker 16 -all_worker 16
#+end_src

** Running RC_2QP benchmarks

Starting the server:
#+begin_src shell
./build/frontend/optmistic_benchmark -ownIp=10.10.1.1 -storage_node -dramGB=10 -all_worker 16 -worker 16 -rcopt
#+end_src

Starting the client:
#+begin_src shell
./build/frontend/optmistic_benchmark -ownIp=10.10.1.2 -run_for_seconds=10 -pinThreads -csvFile="optimistic_benchmark_scaleout.csv" -readratios 100 -rcopt -block_size 64 -worker 16 -all_worker 16
#+end_src
