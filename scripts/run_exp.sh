    ...
    for threads in ${thread_configs[@]}; do
      ...
      while ${retry}; do
        ...
        if [ "$bench_name" == "rcopt" ]; then
          ${BIN} -ownIp=${SERVER_IP} -storage_node -dramGB=10 -all_worker $threads -worker $threads -rcopt &
          s_pid=$!
        else
          ${BIN} -ownIp=${SERVER_IP} -storage_node -dramGB=10 -all_worker $threads -worker $threads &
          s_pid=$!
        fi
